...
export default function PetRecord() {
  ...
  const load = async () => {
    if (!id) return;
    const [petSnap, ownerSnap, bookingSnap, documentSnap, contractSnap] = await Promise.all([
  ...
  return <div className="record-page">
    ...
    <nav className="record-tabs"><button className={tab === "fiche" ? "active" : ""} onClick={() => setTab("fiche")}>Fiche complète</button><button className={tab === "historique" ? "active" : ""} onClick={() => setTab("historique")}>Séjours <b>{bookings.length}</b></button><button className={tab === "documents" ? "active" : ""} onClick={() => setTab("documents")}>Documents <b>{documents.length + contracts.length}</b></button></nav>
    ...
    {tab === "documents" && <section className="record-documents"><label className="document-drop"><input type="file" hidden onChange={(event) => void upload(event)} /><span>↑</span><b>{uploading ? "Envoi du document…" : "Ajouter un document"}</b><small>Ordonnance, carnet, vaccin, carte d’identification…</small></label>{contracts.map((contract) => <article className="document-row" key={contract.id}><span>✦</span><div><b>{contract.contractNumber || "Contrat de garde"}</b><small>{contract.statut === "signé" ? "Signé" : "En attente de signature"}</small></div>{contract.pdfUrl && <a href={contract.pdfUrl} target="_blank" rel="noreferrer">Ouvrir</a>}</article>)}{documents.map((item) => <article className="document-row" key={item.id}><span>⌑</span><div><b>{item.fileName || "Document"}</b><small>{item.category || "Document"}</small></div><a href={item.fileUrl} target="_blank" rel="noreferrer">Ouvrir</a><button onClick={() => void removeDocument(item.id)}>Supprimer</button></article>)}{!documents.length && !contracts.length && <div className="empty-state">Aucun document ajouté pour le moment.</div>}</section>}
  </div>;
}
