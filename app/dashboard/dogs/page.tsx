...
export default function AnimalsPage() {
  ...
  const restoreContract = async (dogId: string) => {
    setRestoringId(dogId); setNotice("");
    try {
      const token = await auth.currentUser?.getIdToken();
      const response = await fetch("/api/admin/pets", { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ dogId }) });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error || "RESTORE_FAILED");
      await load();
      setNotice(result.existing ? "Un contrat initial existe déjà pour ce compagnon." : result.emailSent ? "Le contrat initial a été créé et l’e-mail a été envoyé à la famille." : "Le contrat initial a été créé. L’e-mail n’a pas pu être envoyé : vérifiez l’adresse de la famille.");
    } catch { setNotice("Impossible de préparer le contrat initial pour le moment."); }
